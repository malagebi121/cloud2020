 ...
 * @Author embrace
 * @Description
 * @Date create in  2020/11/28 22:44
 */
@Service
@Slf4j
...
//            Thread.sleep(1000);
            ...
            //TimeUnit.MILLISECONDS.sleep(timeNumber);  //  这个要好用一点
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        return "线程池： " + Thread.currentThread().getName() + " PaymentInfo_timeOut,id: " + id + "\t" + "O(∩_∩)O哈哈~,耗时" + id + "秒钟";
    }



    public String paymentInfo_TimeoutHandler(Integer id) {
        log.info("hystrix  fallback流程");
        return "线程池： " + Thread.currentThread().getName() + "系统繁忙超时,id: " + id + "\t" + "o(╥﹏╥)o超时了,耗时" + id + "秒钟";
    }
     ...
     * 服务的降级->进而熔断->恢复调用链路
     *
     * @param id   东西很多   HystrixCommandProperties  这个类里面有详细的配置信息
     * @return
     */
